...
def call_openai(
    ...
    from openai import OpenAI #import been trong ham
    client = OpenAI(
    api_key=os.getenv("OPENAI_API_KEY"),
    base_url=os.getenv("OPENAI_BASE_URL"),
) #tao 1 client object de gui request den OpenAI API
    start = time.perf_counter() #bat dau do thoi gian
    ...
    latency = time.perf_counter() - start #tinh thoi gian thuc hien
    response_text = response.choices[0].message.content #lay noi dung tra ve
...
def compare_models(prompt: str) -> dict:
    ...
    estimated_cost = (len(gpt4o_response.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    return {
        "gpt4o_response": gpt4o_response,
        "mini_response": mini_response,
        "gpt4o_latency": gpt4o_latency,
        "mini_latency": mini_latency,
        "gpt4o_cost_estimate": estimated_cost
    }
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"]) # tra bang gia, fallback neu model khong co trong bang gia
...
def streaming_chatbot() -> None:
    ...
    history = [] # tao client va history rong

    while True: #tao vong lap cho form chat
        user_msg = input("You: ")
        ...
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
            reply += delta # ghép các chunk lại thành reply hoàn chỉnh

        history.extend([{"role": "user", "content": user_msg}, {"role": "assistant", "content": reply}])

        history = history[-6:] # cắt history còn 3 lượt cuối (6 message)
...
def retry_with_backoff(
    ...
    for attempt in range(max_retries + 1):
        try:
            return fn()  # thử gọi fn()
        except Exception:
            if attempt == max_retries:
                raise  # ném lại exception cuối cùng nếu hết số lần thử

            delay = base_delay * (2 ** attempt)  # tính delay theo exponential backoff
...
def run_assistant(
    ...
    while True:
        if max_turns is not None and num_turns >= max_turns: #dung neu dat gioi han so luong
            break
        ...
        # Goij API với streaming và retry_with_backoff
        ...
        #Ghep cac chunk lai thanh reply hoan chinh
        ...
        total_tokens += count_tokens(user_msg)
        total_tokens += count_tokens(reply)
        ...
        #hoan thanh them 1 luot  hoi thoai

        num_turns += 1

    #tra thong ke toan  phien cuoi cung
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        row = (
            f"{prompt:<40} | {gpt4o_response:<40} | {mini_response:<40} | "
            f"{gpt4o_latency:<15} | {mini_latency:<15}"
        )
        rows.append(row)
